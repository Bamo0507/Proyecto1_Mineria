...
```{r}
matr_depto_cat_mes <- matrimonios_depto %>%
  filter(nivel_geo == "departamento", !is.na(mes))

div_depto_cat_mes <- divorcios_depto %>%
  filter(nivel_geo == "departamento", !is.na(mes))


tabla_mes_matr <- matr_depto_cat_mes %>%
  group_by(mes) %>%
  summarise(total = sum(valor, na.rm = TRUE), .groups = "drop") %>%
  mutate(prop = total / sum(total)) %>%
  arrange(desc(total))

tabla_mes_div <- div_depto_cat_mes %>%
  group_by(mes) %>%
  summarise(total = sum(valor, na.rm = TRUE), .groups = "drop") %>%
  mutate(prop = total / sum(total)) %>%
  arrange(desc(total))
...
```{r}
  # Vector de departamentos fríos
meses_frios_list <- c(10,11,12,1,2)
